...
const Finance = () => {
  ...
  const loadFinanceData = async (targetClubId, pageNum = 1) => {
    try {
      setLoading(true);
      const sumData = await getFinanceSummaryApi(targetClubId).catch(() => null);
      setSummary(sumData);

      if (canEdit) {
        const txData = await getTransactionsApi(targetClubId, pageNum, 8).catch(() => ({ transactions: [], pagination: { page: 1, totalPages: 1 } }));
